    ...
    if request.method == 'POST':
        ...
        if "온도" in user_message:
            ...
            try:
                ...
                response_humidfier = requests.post(RASPI_HUMIDIFIER_API_URL, timeout=20000)
...
@csrf_exempt 
def receive_autofunc_date(request): #라즈베리파이단 서버 측에서 보낸 자동실행시간 받는함수  + db저장
    if request.method == 'POST':
        try:
            data = json.loads(request.body)

            auto_pump_worked_date = data.get('pump_function_at')
            auto_humidifier_worked_date = data.get('humidifier_function_at')
            

            if None in (auto_pump_worked_date, auto_humidifier_worked_date):
                return JsonResponse({"status": "error", "message": "데이터 누락"}, status=400)

            # DB에 저장
            auto_function_date.objects.create(
                pump_function_at = auto_pump_worked_date,
                humidifier_function_at = auto_humidifier_worked_date,
            )
            # 저장 후: 1일 초과된 오래된 데이터 삭제
            deletion_cutoff = timezone.now() - timedelta(days=1)
            SensorData.objects.filter(timestamp__lt=deletion_cutoff).delete()
...
def get_setting_values(request): # 현재 기준 설정값 조회 뷰
    if request.method == 'POST':
        ...
        auto_func_date = auto_function_date.objects.first()
        
        if auto_func_date == None:
            pump_function_at = None
        else: 
            pump_function_at = auto_func_date.pump_function_at.strftime("%Y-%m-%d %H:%M")
        
        if auto_func_date == None:
            humidifier_function_at = None
        else: 
            humidifier_function_at = auto_func_date.humidifier_function_at.strftime("%Y-%m-%d %H:%M")
