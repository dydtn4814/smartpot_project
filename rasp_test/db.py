...
def init_db():
    try:
        ...
        cursor.execute("INSERT OR IGNORE INTO soil_moisture_threshold (id, threshold_value, received_at) VALUES (1, None, ?)", (datetime.now().strftime("%Y-%m-%d %H:%M:%S"),))
        cursor.execute("INSERT OR IGNORE INTO humidity_threshold (id, threshold_value, received_at) VALUES (1,None, ?)", (datetime.now().strftime("%Y-%m-%d %H:%M:%S"),))
        conn.commit()
        print("DB 초기화 완료")
